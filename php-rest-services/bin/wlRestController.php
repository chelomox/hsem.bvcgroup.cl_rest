...
abstract class wlRestController {
     ...
     * If no name is provided in the constructor, the real derived controller class must re-implement the wlRestController::getDefaultName method.
     ...
     * the service controller named <b>math</b>, version <b>v1</b> must have a method <b>addGet</b>.
    ...
    public function call($actionName) {
        ...
        if($actionName) {
            ...
            $authActionName = $fullActionName . 'Allowed';
        }

        $allowed = true;
        if($authActionName && is_callable(array($this, $authActionName))) {
            try {
                $allowed = $this->$authActionName();
            }catch(wlRestException $ex) {
                throw $ex;
            }
        }
        if(!$allowed) {
            throw(new wlRestException('Forbidden', 403));
        }
     ...
     * By default this method just rethrows the exception, but it can be reimplemented in the actual derived controller classes to take more custom actions upon the exception.

...
class wlRestRequest {
    ...
    /**
     * @var mixed data submitted through requests (payload)
     */
    private $_actionParams = null;

    /**
     * @var mixed data submitted through PUT requests
     */
    ...
    public function parse($urlRouteFormat) {
        ...
        $urlParamsValues = wlRestUtils::completeArray(explode('/', $pathInfo), $urlFormatCount);
        $this->_params = array();
        foreach($urlRouteFormat as $k => $paramName) {
            $this->_params[$paramName] = $urlParamsValues[$k];
            unset($urlParamsValues[$k]);
        }

        $this->_method = $this->computeRequestMethod();
        if($this->_method == wlRestRequest::REQUEST_METHOD_PUT) {
            $this->_actionParams = array();
        }elseif($this->_method == wlRestRequest::REQUEST_METHOD_POST) {
            $this->_actionParams = $_POST;
        }elseif($this->_method == wlRestRequest::REQUEST_METHOD_GET || $this->_method == wlRestRequest::REQUEST_METHOD_DELETE) {
            $this->_actionParams = array();
            $getString = $urlParamsValues[$urlFormatCount - 1];
            if($getString) {
                $this->_actionParams = explode('/', $getString);
            }
        }
        if(is_array($this->_actionParams)) {
            $this->_actionParams = array_merge($this->_actionParams, $_GET);
        }
        foreach($urlParamsValues as $urlParamsValue) {
            if(!in_array($urlParamsValue, $this->_actionParams)) {
                $this->_actionParams[] = $urlParamsValue;
            }
        }
        $input = $this->getPutPayload();
        if(is_array($input)) {
            $this->_actionParams = array_merge($this->_actionParams, $input);
        }
    }
    ...
    /**
     * Returns the passed data through PUT request.
    ...
    private function getPutPayload() {
        ...
        $decoded = @json_decode($this->_cachedPutInfo, true);
        return $decoded !== null ? $decoded : $this->_cachedPutInfo;
    }

...
class wlLogger {
    ...
    public function getLogFilePath() {
        ...
        return str_replace('\\', '/', $this->_path . '/' . $this->getLogFileName());
    }
    ...
    public function write($info) {
        ...
        if($this->_type == self::TYPE_HTML)
        {
            ...
            if (is_array($info)) {
                foreach ($info as $inf) {
                    $item.= print_r($inf, true) . "\n";
                }
            }
            else {
                $item.= print_r($info, true) . "\n";
            }
            $item.="\n";
        }
    ...
    /**
     * Returns last time when log file was updated.
     * @return int
     */
    public function getLastTime() {
        $logFile = $this->getLogFilePath();
        $ret = file_exists($logFile) ? filemtime($logFile) : 0;
        return $ret;
    }

    /**
     * Deletes all files in the log directory except the current log file.
     */
    public function clearLogDir() {
        $currentLogFilePath = $this->getLogFilePath();
        $dir = $this->getLogDir();
        if (is_dir($dir)) {
            if ($dh = opendir($dir)) {
